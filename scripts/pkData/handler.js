...
// #TODO
// get this working here, then upload to its own repo and add back here
...
const pkData = (function () {
  ...
  function setLogFunction(fn) {
    if (typeof fn == "function") {
      ...
      // #todo reset to original function instead
      log(`Param was ${typeof fn} (not a function)`)
    }
  }

  function sayHello() {
    log("pkData library loaded - current version: 0.1 (unstable)")
  }
  ...
  /* GENERAL */
  /* global */
  function systemDataReport(sysObj = SYSTEM_OBJECT, intro = "System Data") {
  ...
  function overrideSystemData(systemObj, logImport = true) {
    ...
    SYSTEM_OBJECT = newObj
    if (logImport) { log(systemDataReport(SYSTEM_OBJECT, "System data successfully imported")) }
    return newObj;
  }
  function exportSystemData() {
    return copy(SYSTEM_OBJECT)
  }
  function test_sysMangement() {
    let backup = exportSystemData()
    let initial = Object.keys(SYSTEM_OBJECT).length
    overrideSystemData({ 'foo': 'bar' }, false)
    let temp = Object.keys(SYSTEM_OBJECT).length
    overrideSystemData(backup, false)
    return (initial != temp) && (Object.keys(SYSTEM_OBJECT).length == initial)
  }
  ...
  function getGlobal(type) {
    ...
    if (!SYSTEM_OBJECT[type] || SYSTEM_OBJECT[type].length == 0) { fetchSystemData(type) }
    return SYSTEM_OBJECT[type];
  }
...
const getGroups = pkData.listAll.bind(sys, "groups")
const setGlobal = pkData.updateTypeList
const updateMemberInGlobals = pkData.updateEntry.bind(sys, "member") // first param is the 'this' value
